...
export default function RequisitionUpdateForm(props) {
  const {
    id: idProp,
    requisition: requisitionModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = requisitionRecord
      ? { ...initialValues, ...requisitionRecord }
      : initialValues;
    setRecipientID(cleanValues.recipientID);
    setDepotID(cleanValues.depotID);
    setFulfilled(cleanValues.fulfilled);
    setKits(cleanValues.kits ?? []);
    setCurrentKitsValue("");
    setKitQuantity(cleanValues.kitQuantity ?? []);
  ...
  const [requisitionRecord, setRequisitionRecord] =
    React.useState(requisitionModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Requisition, idProp)
        : requisitionModelProp;
      setRequisitionRecord(record);
    };
    queryData();
  }, [idProp, requisitionModelProp]);
  React.useEffect(resetStateValues, [requisitionRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Requisition.copyOf(requisitionRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "RequisitionUpdateForm")}
      {...rest}
    >
      <TextField
        label="Recipient id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || requisitionModelProp)}
